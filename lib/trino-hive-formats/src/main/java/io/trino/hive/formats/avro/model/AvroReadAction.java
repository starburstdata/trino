...
public abstract sealed class AvroReadAction
        permits
        NullRead,
        BoolRead,
        IntRead,
        ...
        ReadErrorReadAction
{
    public abstract Schema readSchema();

    public abstract Schema writeSchema();

    public static AvroReadAction fromAction(Resolver.Action action)
            throws AvroTypeException
    {
        return switch (action.type) {
            case DO_NOTHING -> switch (action.reader.getType()) {
                case NULL -> new NullRead(action.reader, action.writer);
                case BOOLEAN -> new BoolRead(action.reader, action.writer);
            ...
            throws AvroTypeException
    {
        ...
        verify(Arrays.stream(buildSteps)
                        .mapToInt(fieldAction -> switch (fieldAction) {
                            case DefaultValueFieldRecordFieldReadAction a -> a.getOutputChannel();
                            case ReadFieldAction b -> b.getOutputChannel();
        ...
        return new RecordReadAction(readSchema, writeSchema, buildSteps);
    }

    private static Slice[] getSymbolIndex(Resolver.EnumAdjust action)
            throws AvroTypeException
    {
            ...
            throws AvroTypeException
    {
        ...
        return new WrittenUnionReadAction(writerUnion.reader, writerUnion.writer, writerUnion.unionEquiv, readActions);
    }
