...
public class FastBigintGroupByHash
        implements GroupByHash
{
    private static final int INSTANCE_SIZE = instanceSize(FastBigintGroupByHash.class);
    ...
    public FastBigintGroupByHash(boolean outputRawHash, int expectedSize, UpdateMemory updateMemory, int stepSize)
    {
    ...
    {
        return values[hashPosition];
    }

    private int getGroupId(int hashPosition)
    {
        return groupIds[hashPosition];
    }
        ...
        for (int i = 0; i < stepSize; i++) {
            ...
            if (groupId == -1) {
                ...
                if (getGroupId(hashPosition) == -1) {
                    ...
                    positionBuffer[i] = (hashPosition + 1) & mask;
                }
            }
    ...
    {
        loadGroupIds(stepSize, positionBuffer, groupIdBuffer);
        loadValues(stepSize, positionBuffer, valueBuffer);
    }

    @CompilerControl(DONT_INLINE)
    private void loadValues(int stepSize, int[] positionBuffer, long[] valueBuffer)
    {
        for (int i = 0; i < stepSize; i++) {
            int hashPosition = positionBuffer[i];
    ...
    @CompilerControl(DONT_INLINE)
    private void loadGroupIds(int stepSize, int[] positionBuffer, int[] groupIdBuffer)
    {
        for (int i = 0; i < stepSize; i++) {
            int hashPosition = positionBuffer[i];
