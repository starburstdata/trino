...
public class PositionsAppenderPageBuilder
{
            ...
            PositionsAppenderFactory positionsAppenderFactory)
    {
        ...
        for (int i = 0; i < positionsAppenders.length; i++) {
            ...
            positionsAppenders[i] = positionsAppenderFactory.create(type, pageBuilderStatus.createBlockBuilderStatus(), initialExpectedEntries);
        }
    }
    ...
    public void reset()
    {
        ...
        int expectedPositions = calculateBlockResetSize(declaredPositions);
        for (int i = 0; i < positionsAppenders.length; i++) {
            positionsAppenders[i] = positionsAppenders[i].newState(pageBuilderStatus.createBlockBuilderStatus(), expectedPositions);
        }
    ...
    {
        for (PositionsAppender positionsAppender : positionsAppenders) {
            positionsAppender.prepareProcessingByRow();
        }
    }
}
