...
public class PagePartitioner
{
    ...
    private final PositionsAppenderPageBuilder[] columnWisePageBuilders;
            ...
            PositionsAppenderFactory positionsAppenderFactory)
    {
        ...
        this.columnWisePageBuilders = new PositionsAppenderPageBuilder[partitionCount];
        for (int i = 0; i < partitionCount; i++) {
            columnWisePageBuilders[i] = PositionsAppenderPageBuilder.withMaxPageSize(pageSize, sourceTypes, positionsAppenderFactory);
        }
    ...
    public long getSizeInBytes()
    {
        ...
        long sizeInBytes = 0;
        for (PositionsAppenderPageBuilder pageBuilder : columnWisePageBuilders) {
    ...
    public void partitionPageByColumn(Page page)
    {
        ...
        for (int i = 0; i < partitionFunction.getPartitionCount(); i++) {
            ...
            if (!partitionPositions.isEmpty()) {
                appendToOutputPartition(columnWisePageBuilders[i], page, partitionPositions);
                partitionPositions.clear();
            }
        }

        flushColumnWiseBuffers(false);
    }
    ...
    public void flushColumnWiseBuffers(boolean force)
    {
        try (PagesSerde.PagesSerdeContext context = serde.newContext()) {
            // add all full pages to output buffer
            for (int partition = 0; partition < columnWisePageBuilders.length; partition++) {
                PositionsAppenderPageBuilder partitionPageBuilder = columnWisePageBuilders[partition];
                if (!partitionPageBuilder.isEmpty() && (force || partitionPageBuilder.isFull())) {
                    Page pagePartition = partitionPageBuilder.build();
                    partitionPageBuilder.reset();
