...
public class PagePartitioner
{
    ...
    private final PositionsAppenderPageBuilder[] columnarPageBuilders;
            ...
            PositionsAppenderFactory positionsAppenderFactory)
    {
        ...
        this.columnarPageBuilders = new PositionsAppenderPageBuilder[partitionCount];
        for (int i = 0; i < partitionCount; i++) {
            columnarPageBuilders[i] = PositionsAppenderPageBuilder.withMaxPageSize(pageSize, sourceTypes, positionsAppenderFactory);
        }
    ...
    public long getSizeInBytes()
    {
        ...
        long sizeInBytes = 0;
        for (PositionsAppenderPageBuilder pageBuilder : columnarPageBuilders) {
    ...
    public void partitionPageByColumn(Page page)
    {
        ...
        for (int i = 0; i < partitionFunction.getPartitionCount(); i++) {
            ...
            if (!partitionPositions.isEmpty()) {
                appendToOutputPartition(columnarPageBuilders[i], page, partitionPositions);
                partitionPositions.clear();
            }
        }

        flushColumnarBuffers(false);
    }
    ...
    private void appendToOutputPartition(PositionsAppenderPageBuilder outputPartition, Page page, IntArrayList positions)
    {
        ...
        for (int channel = 0; channel < sourceTypes.length; channel++) {
            ...
            outputPartition.append(channel, positions, partitionBlock);
        }
    }
    ...
    {
        flushColumnarBuffers(force);
        flushRowWiseBuffers(force);
    }

    public void flushRowWiseBuffers(boolean force)
    {
    ...
    public void flushColumnarBuffers(boolean force)
    {
        try (PagesSerde.PagesSerdeContext context = serde.newContext()) {
            // add all full pages to output buffer
            for (int partition = 0; partition < columnarPageBuilders.length; partition++) {
                PositionsAppenderPageBuilder partitionPageBuilder = columnarPageBuilders[partition];
                if (!partitionPageBuilder.isEmpty() && (force || partitionPageBuilder.isFull())) {
                    Page pagePartition = partitionPageBuilder.build();
                    partitionPageBuilder.reset();
                    enqueuePage(pagePartition, partition, context);
