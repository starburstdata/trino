    ...
    private PlanNode split(AggregationNode node, Context context)
    {
        ...
        for (Map.Entry<Symbol, AggregationNode.Aggregation> entry : node.getAggregations().entrySet()) {
            ...
            finalAggregation.put(
                    ...
                    new AggregationNode.Aggregation(
                            resolvedFunction,
                            buildFinalAggregationArguments(node.isGlobalAggregation(), originalAggregation, intermediateSymbol),
                            false,
                            Optional.empty(),
                            Optional.empty(),
                            node.isGlobalAggregation() ? Optional.empty() : originalAggregation.getMask()));
        }

        Optional<Symbol> rawInputMaskSymbol = node.isGlobalAggregation() ?
                Optional.empty() :
                Optional.of(context.getSymbolAllocator().newSymbol("rawInputMaskSymbol", BOOLEAN));
